import { z } from 'zod';

const FlightInfoSchema = z.object({
  airline: z.string(),
  departure: z.object({
    date: z.string(),
    time: z.string(),
  }),
  arrival: z.object({
    date: z.string(),
    time: z.string(),
  }),
  price: z.string(),
  returnFlight: z.object({
    departure: z.object({
      date: z.string(),
      time: z.string(),
    }),
    arrival: z.object({
      date: z.string(),
      time: z.string(),
    }),
  }).optional(),
  isBestValue: z.boolean().optional(),
});

export const MessageBaseSchema = z.object({
  type: z.enum(['human', 'agent', 'conflict']),
  content: z.string(),
  summary: z.string().optional(),
  questions: z.array(z.string()).optional(),
  has_view_plan: z.boolean().default(false),
  plan_version_id: z.string().nullable().optional(), // Link to plan version generated by this message
  flights: z.array(FlightInfoSchema).optional(), // Structured flight data for UI display
});

export const MessageCreateSchema = MessageBaseSchema.extend({
  type: z.enum(['human', 'agent']).default('human'),
});

export type MessageBase = z.infer<typeof MessageBaseSchema>;
export type MessageCreate = z.infer<typeof MessageCreateSchema>;

export interface MessageInDB extends MessageBase {
  _id?: string;
  tripId?: string;
  authorId?: string;
  conflictId?: string;
  planVersionId?: string; // Link to plan version
  createdAt?: Date;
}

export interface Message extends MessageBase {
  id: string;
  trip_id: string;
  author_id: string | null;
  conflict_id: string | null;
  plan_version_id?: string | null;
  created_at: Date;
}
